...
fun htmlToJson(inPath: File, outPath: File) {
    val messages = mutableListOf<Chat.Message>()
    lateinit var from: String

    var i = 0
    while (true) {
        val htmlFile = File(inPath, "messages${if (i != 0) i + 1 else ""}.html")
        ...
        Jsoup.parse(htmlFile, null).select(".message.default").forEach {
            if (it.`is`(".default")) {
                ...
                it.selectFirst(".photo_wrap")?.let {
                    photo = File(inPath, it.attr("href")).also {
                    ...
                    file = File(inPath, it.attr("href"))
                    thumbnail = File(inPath, it.selectFirst(".animated")!!.attr("src"))
                    mediaType = "animation"
                } ?: it.selectFirst(".media_video")?.let {
                    file = File(inPath, it.attr("href"))
                    mediaType = "animation"
                } ?: it.selectFirst(".video_file_wrap")?.let {
                    file = File(inPath, it.attr("href")).also {
                    ...
                    thumbnail = File(inPath, it.selectFirst(".video_file")!!.attr("src"))
                    ...
                    thumbnail = File(inPath, it.selectFirst(".sticker")!!.attr("src"))
                    mediaType = "sticker"
                    stickerEmoji = ""
                ...
                messages += Chat.Message(it.attr("id").substring(7).toInt(), "message", jsonDateTimeFormatter.format(htmlDateTimeFormatter.parse(it.selectFirst(".date.details")!!.attr("title"))), from/*, "user"*/, file?.toRelativeString(outPath)?.replace('\\', '/'), thumbnail?.toRelativeString(outPath)?.replace('\\', '/'), mediaType, stickerEmoji, photo?.toRelativeString(outPath)?.replace('\\', '/'), null, durationSeconds, width, height, it.selectFirst(".text")?.let {
                    StringBuilder().apply {
    ...
    jacksonObjectMapper().writeValue(File(outPath, "result.json"), Chat("personal_chat", messages))
}
